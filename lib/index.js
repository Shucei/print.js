...
class Print {
  ...
  getHtml () {
    ...
    // 添加水印
    if (this.watermark) {
      outerHTML = `<div>${outerHTML}<div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) rotate(-35deg); color: rgba(200, 200, 200, 0.2); font-size: 20vw;white-space: nowrap;">${this.watermark}</div></div>`;
    }
    return outerHTML;
